...
namespace Mono.Nat.Upnp
{
    ...
    {
        public static ResponseMessage Decode(UpnpNatDevice device, string message, bool isErrorMessage = false)
        {
            try
            {
            ...
            {
                if (isErrorMessage)
                    NatUtility.Log("Exception while processing error message.\n" + x.ToString());
                else
                    NatUtility.Log(x.ToString());
                NatUtility.Log(message);
                return null;
            }
        }
